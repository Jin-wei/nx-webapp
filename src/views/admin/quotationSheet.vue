                        ...
                        <form class="form-horizontal">
                            <div class="quo-top-border">
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-2">
                                        <input v-model="searchData.realName" type="text" class="form-control input-sm" id="inputID" oninput="if(value.length>30)value=value.slice(0,30)" >
                                    </div>
                    ...
                    <form id="importForm">
                        <div class="modal-body" style="min-height: 350px">
                            <div class="form-group col-md-12">
                                <label >是否是公共报价：</label>
                                <select class="form-control"  v-model="offer.type" @change="changeOffer">
                            ...
                            <div class="form-group col-md-12">
                                <label >客户名称：</label>
                                <input class="form-control col-md-8" v-model="offer.name" id="CustomerName"  maxlength="50" data-parsley-maxlength="50" :disabled=offerInputDisable v-on:keyup="getCustomerName">
                            </div>
                            <div class="form-group col-md-12">
...
<script>
    ...
    const apiUrl = '/api/admin/quotationSheet?method='
    ...
    export default {
        ...
        mounted: function() {
            ...
            function actFormatter(value, row) {
                if(row.status===0 && row.latest==1 ){
                    return [
                        '<span class="label label-success operation-margin btn-submit" >  提交  </span>'+
                        '<span class="label label-info operation-margin btn-edite" >  编辑  </span>'+
                ...
                }else if(row.status==1 && row.latest==1 ){
                    return [
                ...
                else if(row.latest==0 && row.status===1){
                    return [
                        '<span class="label label-success operation-margin btn-submit"  style="cursor: not-allowed; opacity: 0.5;   ">  提交  </span>'+
                        '<span class="label label-info operation-margin btn-edite" style="cursor: not-allowed; opacity: 0.5;   " >  编辑  </span>'+
                        '<span class="label label-warning operation-margin btn-delete">  删除  </span>'+
                        '<span class="label label-primary operation-margin btn-detail" >  详情  </span>'
                    ].join('')
                }
                else if(row.latest==0 && row.status===0){
                    return [
                        '<span class="label label-success operation-margin btn-submit" >  提交  </span>'+
                        '<span class="label label-info operation-margin btn-edite" style="cursor: not-allowed; opacity: 0.5;   " >  编辑  </span>'+
                        '<span class="label label-warning operation-margin btn-delete">  删除  </span>'+
            ...
            function initTable() {
                window.tableEvents = {
                    'click .btn-submit': function(e, value, row, index) {
                        if(row.status===0){
                            /*common.rowModifyWithT(_self, apiUrl + 'confirmQuotation', row, 'id', $('#table'))*/
                    ...
                    'click .btn-delete': function(e, value, row, index) {
                        common.dealConfrimCommon('确认删除此报价？', function() {
                            _self.$http.post(apiUrl + 'deleteQuotation', row).then((response) => {
                    ...
                    'click .btn-edite': function(e, value, row, index) {
                        if(row.latest==1){
                            _self.$router.push({path: '/admin/quotationEditSheet', query: {id: row.id}});
                        }
                        /*_self.$router.push({ path: url , query: {purchaseId: row.srm_purchaseorder_id}})*/
                    },
                    'click .btn-detail': function(e, value, row, index) {
                        _self.$router.push({path: '/admin/quotationDetailSheet', query: {id: row.id}})
                    }
                ...
                $('#table').bootstrapTable({
                    ...
                    columns: [
                        ...
                        common.BTRowFormatWithFormatter('real_name', '客户名称', nameFormatter),
                        common.BTRowFormat('count', '报价明细数量'),
                        common.BTRowFormat('salesName', '所属业务员'),
                        common.BTRowFormatWithFormatter('latest', '是否最新',commonFormatter),//0不是最新 1最新
                        common.BTRowFormatWithFormatter('is_common', '是否公共报价',commonFormatter),//0不公共 1公共
...
//                        common.BTRowFormatWithFormatter('c_time', '创建时间',timeFormatter),
        ...
        methods: {
            ...
            changeOffer:function () {
                let _self=this;
                if(_self.offer.type==='1'){
                    _self.offerInputDisable=true;
                }else if(_self.offer.type==='0'){
                    _self.offerInputDisable=false;
                }
            },
            ...
            getCustomerName:function () {
                ...
                $("#CustomerName").autocomplete({
                    ...
                    select: function( event, ui ) {
                        $( "#select1" ).val( ui.item.customer_id );
                        _self.offer.name=ui.item.real_name;
                        return false;
                    }
            ...
            importFile: function () {
                ...
                let params="";
                if(_self.offer.type=='0'){
                    if($( "#select1" ).val()=="" || $( "#select1" ).val()==null){
                        common.dealPromptCommon('客户名称不能为空！')
                        return
                    }
                    params={
                        isCommon:_self.offer.type,
                        saveType:0,
